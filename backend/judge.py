...
1. 若配置了 LLM_* 环境变量且可动态导入 httpx → 调用 OpenAI 兼容接口判题（source='llm'）；
2. 否则降级为启发式判题（基于参考答案关键词覆盖率，source='heuristic'）；
3. 任何异常 → 返回 (None, '判题服务暂不可用', 'judge_error', 'pending')。
...
async def _llm_judge(question_text: str, reference_answer: str, user_answer: str):
    """可选 LLM 判题；不可用时返回 None（交由 heuristic）。"""
