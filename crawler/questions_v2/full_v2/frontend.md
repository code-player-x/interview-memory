...
> 题目数量：**431** ｜ 渲染时间：自动 ｜ 源：authored/frontend.jsonl
...
## 227. 对话状态用 Pinia 还是用 Composition API？
...
## 228. 如何理解reactive、ref 、toRef 和 toRefs？
...
## 231. flex-wrap属性定义，如果一条轴线排不下，如何换行？
...
## 233. Canvas 和 SVG 都是用于在 web 页面上进行图形绘制的技术，它们各自具有不同的特点和适用场景。下面是它们的主要区别？
...
## 238. 描述cookie，localStorage，sessionStorage的区别？
...
## 239. load和 DOMContentLoaded 的区别？
...
## 240. HTML 的 meta 标签有哪些？分别代表什么意思？
...
## 251. TypeScript 和 JavaScript 是什么关系？TS 的优势在哪里？
...
## 253. 父类上的引用类型的属性，子类继承下来，还是那个引用类型吗？
...
## 259. 问几个简单点的问题：watch 和 created 谁先执行？
...
## 263. Redux 的 dispatch 如何将 action 交给对应的 reducer 处理？
...
## 264. 用 const 定义了一个 对象后，能用另一个对象替换吗？
...
## 265. 网络请求的源码？ 比如有看过 axios 源码，封装过吗？
...
## 266. 在setTimeout 里产生一个promise，当前的promise 会在当前 次执行掉吗？还是在下一次循环里执行？
...
## 268. 实现 debounce，调用方式 debounce(fn, duration, leading)？
...
## 269. JSONP和ajax有什么区别，手写一个JSONP(promise版的)，window上挂在的函数如何避免重名？
...
## 271. 基础原理 八股文不用背，但Promise/事件循环/闭包的原理得懂？
...
## 288. 你有没有做登陆态持久化，怎么做的（设置cookie过期时间）？
...
## 294. 移动端适配？（媒体查询，flex，rem）还有viewport
...
## 295. 所有绝对居中的实现 ？( flex, text, padding: auto）没答全，还有translate？
...
## 297. AntV X6 具体承担什么？节点和边的数据结构怎么设计？
...
## 305. 页面非常卡顿，卡顿不是因为数据很多，而是因为有很多计算，导致掉帧了，如何找出卡顿的原因？
...
## 306. setTimeout延时写成0，我们一般什么情况会这么做？
...
## 308. newVnode和oldVnode都有子节点，通过updateChildren对比子节点？
...
## 310. 有了解过 你们渲染引擎是 怎么解析你们这个 json schema 的吗？
...
## 314. 移动端开发一般怎么判断是什么平台，安卓、IOS、微信？
...
## 316. 为什么请求放在useEffect里，放在外面和放里面有什么区别？
...
## 317. 我对比了网上几种不错的静态网站生成器后，发现使用 VuePress 搭建知识体系框架很合我的胃口！？
...
## 318. 为什么手写 Promise/EventEmitter 的考察重点变了？
...
## 319. AI 生成的 Promise 实现有边界 case 问题，你能找出来并解释吗？
...
## 321. 题目：遍历一个任意长度的list中的元素并依次创建异步任务，如何获取所有任务的执行结果？
...
## 328. 动态表单联动怎么做？例如 A 字段选择“报销”，B 字段才显示，C 字段变为必填？
...
## 332. 标准盒模型和IE盒模型的区别在于设置width和height时，对应的范围不同？
...
## 333. em和rem，相对长度单位，它们之间的区别：em相对于父元素，rem相对于根元素？
...
## 340. 一旦父节点元素应用了 display:none，父节点及其子孙节点元素全部不可见，而且无论其子孙元素如何不屈地挣扎都无济于事？
...
## 346. 如何理解和使用Promise.all和Promise.race？
...
## 358. patch方法：对比当前同层的虚拟节点是否为同一种类型的标签？
...
## 360. route和router都是与路由相关的对象，但是它们扮演的角色不同，有以下区别？
...
## 366. 需要在本地实现一个聊天室，多个tab页相互通信，不能用websocket，你会怎么做？
...
## 372. 能在这两个声明周期里面通过this拿到method，data，$refs这些吗？
...
## 379. 父元素设置 opacity 后，为什么子元素不能通过 opacity: 1 恢复不透明？
...
## 381. 文字的大小，基础的能力都有了，那业务方希望说我希望边框是曲线的边框，这时候你应该怎么办？
...
## 383. 什么业务场景下，会用到错误边界，它用在哪个地方会比较好？
...
## 385. 定义 多个 reducers ， reducers是一个什么概念？
...
## 388. 假设我的 promise 里面全部都是同步代码，promise里面没有做任何异步，不写 pending 是不是也可以？
...
## 392. 如果有一个外部函数内部返回一个内部函数，内部函数引用了外部函数的变量，产生闭包，如果这个外部函数被调用 10 次，会产生多少个闭包呢？
...
## 393. 说一下 useLayoutEffect 和 useEffect 的区别是什么？
...
## 394. 对 useCallback、useMemo 这两个 hook 的理解，有什么样的区别，适合在什么场景下使用？
...
## 395. 在useEffect里想使用async/await怎么用？
...
## 397. ReadableStream 的 reader 是怎么处理半截 JSON 的？
...
## 401. 🌈不仅要为不同的页面组显示不同的侧边栏，还想将侧边栏划分为多个组。如何实现？
...
## 402. 性能优化后为什么还要再跑一次 Lighthouse 对比数据？
...
## 406. mouseEnter 和 mouseOver 有什么区别？
...
## 410. 经过我们的不断尝试，最终确认通过使用“AE+Lottie”方案实现模版的制作和播放，使用“pupteer+ffmpge”生成最终的视频文件？
...
## 429. fix的显示问题？（宽度塌陷）没答全，还有z-index覆盖？
