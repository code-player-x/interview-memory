...
## 9. 深拷贝、浅拷贝、结构化数据更新在 Vue 状态管理里分别会遇到什么坑？
...
## 10. Pinia 里哪些状态应该放 store，哪些应该留在组件内部？
...
## 11. 条件分支怎么表达？如果两个分支条件冲突，前端要不要校验？
...
## 13. WebSocket 连接怎么在 Vue 组件中优雅管理？
...
## 21. 实现方式：Proxy是ES6新增的一种特性，使用了一种代理机制来实现响应式？
...
## 23. 为什么 JavaScript 中 0.1 + 0.2 不严格等于 0.3？应如何处理浮点误差？
...
## 24. JavaScript 基本类型为什么能访问属性和方法？装箱与拆箱机制是什么？
...
## 25. link 与 @import 引入 CSS 有什么区别？应如何选择？
...
## 41. CSS 隐藏、脱离文档流与真正移除 DOM 的状态、事件和性能差异是什么？
...
## 47. 描述vue组件生命周期（有父子组件的情况呢），每个生命周期都做了什么？
...
## 54. webpack的rule，loader的执行流是怎么样的？
...
## 55. 如何从零初始化一个 Vue 3 项目，并设计工具链与目录结构？
...
## 57. Vue 2 与 Vue 3 在响应式、组织方式和性能上有什么区别？
...
## 61. webpack或者vite中可以使用proxy解决跨域，它解决跨域的原理是什么？
...
## 62. 分别为一个组件设置v-if和v-show，值由false变成true，或者由true变成false，组件的生命周期会怎么执行？
...
## 66. 有做过工程化的事情吗？说一下你对工程化的了解，对webpack的了解？
...
## 68. Vue 组件触发 emit 后，父组件监听回调与后续代码的执行顺序如何确定？
...
## 71. webpack的module、bundle、chunk分别指的是什么？
...
## 75. Vue 的 nextTick 其本质是对 JavaScript 执行原理 EventLoop 的一种应用？
...
## 76. 实现一个左右布局，左侧200px，右侧自适应，css写有几种方式：我说了浮动、定位、弹性盒、网格这四种？
...
## 78. 组件库会有一个像 modal 的这个组件，有没有想过如何实现一个通用的这个组件？
...
## 79. ok，聊一下ES6的promise吧，它的静态方法的区别：all、race、allsettled、any？
...
## 80. 如何根据受校验的 Schema 将结构化描述安全地渲染为组件树？
...
## 82. 低代码平台如何用 JSON Schema 定义组件间的数据与事件联动？
...
## 83. 低代码编辑器如何实现组件拖拽、边界检测、吸附与自动落位？
...
## 91. 前端如何处理敏感数据？加密、脱敏与访问控制的边界是什么？
...
## 93. 自己有对 webpack 了解吗？ webpack的整体流程？
...
## 95. 讲一下低代码这个系统是怎么设计、怎么实现的？在实现上有哪些难度啊？
...
## 99. 就你不可能直接使用原生DOM进行操作把？总归是有一些封装的吧？代码是怎么组成的？拖拽这个能力作为一个比较独立的功能，你们没有任何封装？
...
## 101. 低代码平台通常交付哪些产物？如何从 Schema 生成可运行页面？
...
## 102. 低代码画布中组件宽高如何记录？应选择 px、百分比还是相对单位？
...
## 103. 有没有说一些 解耦方面的 或者说是 复用方面的 做得比较好的组件 可以讲一下？
...
## 104. React 在什么情况下更新虚拟 DOM 树？render 阶段如何工作？
...
## 106. 合并数据：比如一个组件可以有1,2,3个版本，会有渲染的时候可以选择哪一个版本的组件，但是它里边内部的数据，应该不会去区分把？
...
## 107. 低代码平台的编辑态与预览态有哪些差异？如何共享 Schema 与渲染逻辑？
...
## 108. 编辑态的时候，比如说左侧肯定会有一些组件，拖拽的时候会拖入到画布区，你们是怎么渲染出来的？比如用iframe 还是直接渲染出来的？
...
## 109. 脚手架工具可以简单介绍一下吗？专门对外的一个组件库开发脚手架？
...
## 110. 低代码组件开发完成后，交付给平台的产物包包含哪些内容？
...
## 115. 什么情况下需要为前端项目自定义 Webpack 配置？
...
## 116. webpack 是怎么处理 commonjs/esm？
...
## 117. 前端构建中应如何分别处理 CSS、JavaScript、图片等资源？
...
## 119. DOM 解析？CSS 解析？JS 也需要解析，你觉得这几个是什么样的流程？
...
## 120. webpack treeShaking原理，是靠什么才能实现(ES6模块的静态导出)？
...
## 122. Vue、VuePress 与 VitePress 是什么关系？为什么它们的文档站风格相似？
...
## 123. tailwind 原子类名负责单一属性，假设我想 font-weight、font-size 结合用一个类名怎么做？
...
## 125. 场景问题：比如支付宝开福，从前端角度如何优化来提高用户体验，提示点：开福不考虑先来后到？
...
## 130. 子组件是一个 Portal，发生点击事件能冒泡到父组件吗？
...
## 140. js里的多重继承怎么实现（call，es6 extend)？
...
## 143. webpack编译和构建原理（分析依赖，chunk）没有说loader？
...
## 146. Vue 项目何时需要 jQuery？如何避免与响应式 DOM 管理冲突？
...
## 149. 前端应用如何设计可靠的 undo/redo？状态快照与命令模式如何取舍？
...
## 154. 监听一个数组，数组变更了，那么数组的函数怎么会达到二次执行？
...
## 157. 数组的length是2，forEach的时候，长度增加为10了，forEach会重复执行吗？如果会那vue会怎么去做的？
...
## 159. 对类组件和函数组件的理解，它们的区别，什么情况下写类组件更好，什么情况下写函数组件更好？
...
## 163. JavaScript 有哪些原始类型？应如何准确判断数据类型？
...
## 164. display:none和visibility:hidden的区别？
...
## 171. updateChildren方法: 新旧虚拟节点的子节点对比？
...
## 173. 有了解composition api底层怎么实现的吗？
...
## 175. defer 与 async 脚本的加载和执行时机有什么区别？
...
## 176. 单页应用（SPA）与多页应用（MPA）有什么区别？如何选型？
...
## 177. 组件的设计规范：比如说你设计一个button，怎么样能够设计出一个好的 button？
...
## 178. CommonJS和ES6模块化有什么区别，设计一个方法，让CommonJS导出的模块也能改变其内部变量？
...
## 180. 原生 H5 的优势与局限是什么？何时应引入框架或工具库？
...
## 181. Promise、async/await 的关系是什么？如果 async 函数里 throw error，外层怎么捕获？
...
## 182. EventSource 与 fetch + ReadableStream 有什么区别？如何选型？
...
## 183. Composition API 相比 Options API 的工程收益与适用边界是什么？
...
## 184. Vue created 中发起异步请求后，回调里何时能访问组件状态和 DOM？
...
## 185. 异步获取内容高度未知时，前端如何稳定布局并避免 CLS？
...
## 186. React Hooks 如何在状态更新后触发重新渲染？
...
## 187. Promise 的状态机、链式调用与错误传播机制是什么？
...
## 188. v-if 与 v-show 有什么区别？应如何按渲染与切换成本选型？
...
## 189. AI 数据可视化：通过 Recharts 将 AI 分析后的简历评分、多维对比以直观的雷达图形式呈现，让数据“会说话”？
...
## 190. margin(top,left,right,bottom)设置负值有何效果？
...
## 191. 你脚手架那块的话，我理解是说，可以通过指令生成一些项目的模板，是这样子的吗？
...
## 192. 项目中如何使用 TypeScript 保障接口与状态的类型安全？
...
## 193. 浏览器的重排（layout/reflow）与重绘分别是什么？如何减少它们的代价？
...
## 194. LCP 从 3.7s 降到 1.8s，你具体做了哪些优化？怎么测量？
...
## 196. optimize方法对AST树进行静态内容优化，分析出哪些是静态节点，给其打一个标记，为后续更新渲染可以直接跳过静态节点做优化？
...
## 197. three.js 场景的射线检测和加速结构应如何选择？
...
## 199. 在什么情况下判断预加载（点击时利用150ms延迟进行预加载）？
...
## 200. JavaScript 条件运算符需要哪些组成部分？如何选择两套阈值？

> 原题 ID：`q1639`

**高频程度**：★★★

**考察点**：恢复缺失的条件表达式，并区分条件、真分支和假分支。

**回答框架**：

1) 完整形式是 condition ? whenTrue : whenFalse
2) 先计算条件的 truthy/falsy，再只求值一个分支
3) heavy/light 的选择条件必须由业务显式给出
4) 复杂分支改用 if/else 提升可读性
...
**常见追问**：条件运算符的两个分支都会执行吗？这对包含函数调用的分支有什么影响？

---

## 201. EventSource.close() 有什么作用？应在何时调用？
...
## 203. TypeScript 里的 type 和 interface 有什么区别？
...
## 205. 可视化平台如何通过中间表示和适配层与不同渲染引擎通信？
...
## 206. SameSite=Strict、Lax、None 分别在什么跨站请求场景下携带 Cookie？
...
## 207. 如何解释 Vue 的 v-show，以及它与 v-if 的选型边界？
...
## 208. 前端应用为何需要本地持久化？它如何减轻网络重传与服务端压力？
...
## 209. 前端本地持久化有哪些实现方式？localStorage 的边界是什么？
...
## 210. 如何选择或集成 WYSIWYG 编辑器，并处理安全、扩展和内容存储？
...
## 211. meta viewport 与 http-equiv 分别有什么作用和使用边界？
...
## 215. 对话状态用 Pinia 还是用 Composition API？
...
## 216. 如何理解reactive、ref 、toRef 和 toRefs？
...
## 219. flex-wrap属性定义，如果一条轴线排不下，如何换行？
...
## 221. Canvas 和 SVG 都是用于在 web 页面上进行图形绘制的技术，它们各自具有不同的特点和适用场景。下面是它们的主要区别？
...
## 226. 描述cookie，localStorage，sessionStorage的区别？
...
## 227. load和 DOMContentLoaded 的区别？
...
## 228. HTML 的 meta 标签有哪些？分别代表什么意思？
...
## 239. TypeScript 和 JavaScript 是什么关系？TS 的优势在哪里？
...
## 241. 父类上的引用类型的属性，子类继承下来，还是那个引用类型吗？
...
## 247. 问几个简单点的问题：watch 和 created 谁先执行？
...
## 251. Redux 的 dispatch 如何将 action 交给对应的 reducer 处理？
...
## 252. 用 const 定义了一个 对象后，能用另一个对象替换吗？
...
## 253. 网络请求的源码？ 比如有看过 axios 源码，封装过吗？
...
## 254. 在setTimeout 里产生一个promise，当前的promise 会在当前 次执行掉吗？还是在下一次循环里执行？
...
## 256. 实现 debounce，调用方式 debounce(fn, duration, leading)？
...
## 257. JSONP和ajax有什么区别，手写一个JSONP(promise版的)，window上挂在的函数如何避免重名？
...
## 259. 基础原理 八股文不用背，但Promise/事件循环/闭包的原理得懂？
...
## 275. 你有没有做登陆态持久化，怎么做的（设置cookie过期时间）？
...
## 281. 移动端适配中，viewport、媒体查询、Flex、rem 与 vw 分别解决什么问题？
...
## 282. 所有绝对居中的实现 ？( flex, text, padding: auto）没答全，还有translate？
...
## 284. AntV X6 具体承担什么？节点和边的数据结构怎么设计？
...
## 292. 页面非常卡顿，卡顿不是因为数据很多，而是因为有很多计算，导致掉帧了，如何找出卡顿的原因？
...
## 293. setTimeout延时写成0，我们一般什么情况会这么做？
...
## 295. newVnode和oldVnode都有子节点，通过updateChildren对比子节点？
...
## 297. 有了解过 你们渲染引擎是 怎么解析你们这个 json schema 的吗？
...
## 301. 移动端开发一般怎么判断是什么平台，安卓、IOS、微信？
...
## 303. 为什么请求放在useEffect里，放在外面和放里面有什么区别？
...
## 304. 我对比了网上几种不错的静态网站生成器后，发现使用 VuePress 搭建知识体系框架很合我的胃口！？
...
## 305. 为什么手写 Promise/EventEmitter 的考察重点变了？
...
## 306. AI 生成的 Promise 实现有边界 case 问题，你能找出来并解释吗？
...
## 308. 题目：遍历一个任意长度的list中的元素并依次创建异步任务，如何获取所有任务的执行结果？
...
## 315. 动态表单联动怎么做？例如 A 字段选择“报销”，B 字段才显示，C 字段变为必填？
...
## 319. 标准盒模型和IE盒模型的区别在于设置width和height时，对应的范围不同？
...
## 320. em和rem，相对长度单位，它们之间的区别：em相对于父元素，rem相对于根元素？
...
## 327. 一旦父节点元素应用了 display:none，父节点及其子孙节点元素全部不可见，而且无论其子孙元素如何不屈地挣扎都无济于事？
...
## 333. 如何理解和使用Promise.all和Promise.race？
...
## 345. patch方法：对比当前同层的虚拟节点是否为同一种类型的标签？
...
## 347. route和router都是与路由相关的对象，但是它们扮演的角色不同，有以下区别？
...
## 353. 需要在本地实现一个聊天室，多个tab页相互通信，不能用websocket，你会怎么做？
...
## 359. 能在这两个声明周期里面通过this拿到method，data，$refs这些吗？
...
## 366. 父元素设置 opacity 后，为什么子元素不能通过 opacity: 1 恢复不透明？
...
## 368. 文字的大小，基础的能力都有了，那业务方希望说我希望边框是曲线的边框，这时候你应该怎么办？
...
## 370. 什么业务场景下，会用到错误边界，它用在哪个地方会比较好？
...
## 372. 定义 多个 reducers ， reducers是一个什么概念？
...
## 374. Promise 为什么要区分 pending、fulfilled 和 rejected 状态？
...
## 375. 假设我的 promise 里面全部都是同步代码，promise里面没有做任何异步，不写 pending 是不是也可以？
...
## 379. 如果有一个外部函数内部返回一个内部函数，内部函数引用了外部函数的变量，产生闭包，如果这个外部函数被调用 10 次，会产生多少个闭包呢？
...
## 380. 说一下 useLayoutEffect 和 useEffect 的区别是什么？
...
## 381. 对 useCallback、useMemo 这两个 hook 的理解，有什么样的区别，适合在什么场景下使用？
...
## 382. 在useEffect里想使用async/await怎么用？
...
## 384. ReadableStream 的 reader 是怎么处理半截 JSON 的？
...
## 388. 🌈不仅要为不同的页面组显示不同的侧边栏，还想将侧边栏划分为多个组。如何实现？
...
## 389. 性能优化后为什么还要再跑一次 Lighthouse 对比数据？
...
## 393. mouseEnter 和 mouseOver 有什么区别？
...
## 397. 经过我们的不断尝试，最终确认通过使用“AE+Lottie”方案实现模版的制作和播放，使用“pupteer+ffmpge”生成最终的视频文件？
